...
export default function RecruiterCandidatesList() {
  ...
  const fetchMatchedJobs = async (candidate: any) => {
    setJobsLoading(true);
    try {
      const res = await axios.get("https://adextravelnursing.com/api_get_jobs_for_candidates.php");
      const allJobs = Array.isArray(res.data) ? res.data : [res.data];

      // console.log("All Jobs:", allJobs);

      // Ensure candidate fields are arrays
      const candidateSpecialty = Array.isArray(candidate.specialty) ? candidate.specialty : [];
      const candidateShifts = Array.isArray(candidate.preferred_shift) ? candidate.preferred_shift : [];
      const candidateLocations = Array.isArray(candidate.preferred_location) ? candidate.preferred_location : [];


      const matched = allJobs.filter((job: any) => {
        // Ensure job fields are arrays
        const jobSpecialty = Array.isArray(job.job_specialty) ? job.job_specialty : [];
        const jobShift = Array.isArray(job.job_shift) ? job.job_shift : [];
        const jobLocation = Array.isArray(job.job_state) ? job.job_state : [];
        const jobDegree = job.job_degree ? job.job_degree.toLowerCase() : "";

        // Partial match (like) for specialty
        const specialtyMatch = candidateSpecialty.some((s: string) =>
          jobSpecialty.some((js: string) =>
            js.toLowerCase().includes(s.toLowerCase())
          )
        );

        // Partial match (like) for shift
        const shiftMatch = candidateShifts.some((shift: string) =>
          jobShift.some((js: string) =>
            js.toLowerCase().includes(shift.toLowerCase())
          )
        );

        // Partial match (like) for location
        const locationMatch = candidateLocations.some((loc: string) =>
          jobLocation.some((jl: string) =>
            jl.toLowerCase().includes(loc.toLowerCase())
          )
        );

        const degreeMatch =
          candidate.degree &&
          jobDegree.includes(candidate.degree.toLowerCase());

        return specialtyMatch && shiftMatch && locationMatch && degreeMatch;
      });
  ...
  return (
    <>
      ...
      <TabGroup index={activeTabIndex} onIndexChange={setActiveTabIndex} className="mt-2">
        <TabList>
          <Tab className={`px-4 py-2 rounded-md ${activeTabIndex === 0 ? 'bg-sky-900 text-white' : ''}`}>Candidates</Tab>
          <Tab className={`px-4 py-2 rounded-md ${activeTabIndex === 1 ? 'bg-sky-900  text-white' : ''}`} disabled={!selectedCandidate}>Details</Tab>
          <Tab className={`px-4 py-2 rounded-md ${activeTabIndex === 2 ? 'bg-sky-900  text-white' : ''}`}>Add New</Tab>
        </TabList>
                      ...
                      {matchedJobs.length === 0 ? (
                        ...
                        <ul className="space-y-4">
                          {matchedJobs.map(job => (
                            ...
                            <li key={job.job_id} className="flex justify-between items-center p-4 border rounded-md border-gray-400 cursor-pointer">
                              <div className="">
                                <span className='text-[10px] text-gray-600'><strong>ID: </strong>{job.job_id}</span>
                                <p className='li_bold font-bold'>{job.job_title}</p>
                                <span>{job.job_specialty} | {job.job_shift} | {job.job_city}, {job.job_state}</span>
                              </div>
                              <div className="">
